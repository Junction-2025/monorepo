...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert event camera .dat file to .ply point cloud"
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def load_events_from_dat(
    input_file: Path, width: int, height: int
...
def calculate_z_coordinates(
    timestamps: np.ndarray, use_timestamp_as_z: bool, normalize_time: bool
) -> np.ndarray:
...
def build_point_cloud_dataframe(
    xs: np.ndarray,
    ys: np.ndarray,
    zs: np.ndarray,
    red: np.ndarray,
    ...
    return pd.DataFrame(
        {"x": xs, "y": ys, "z": zs, "red": red, "green": green, "blue": blue}
    )
...
def main():
    ...
    xs, ys, ts, pols = load_events_from_dat(args.input, args.width, args.height)

    zs = calculate_z_coordinates(ts, args.use_timestamp_as_z, args.normalize_time)
