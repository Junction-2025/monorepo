    ...
    for batch_range in pacer.pace(src.ranges()):
        ...
        if len(roi_signals) >= max_signals:
            ...
            event_data = {
                ...
                "rpm": rpm,
                "rotors": blade_count,
                "description": f"Detected a drone. The drone model and number of rotors is simulated for demo purposes",
                "timestamp": datetime.datetime.now().isoformat()
            }
            requests.post("http://localhost:8000/trigger_event", json=event_data)
            roi_signals.clear()        
        """
