...
- We implemented a profiler through `src/profiling.py` in order to estimate running times of individual running blocks of the code. It runs on the logic of time substraction between the start of the function call and the end of the function call. It gave us hints when certain algorithmical solutions became too slow for the context of the challenge, helping us to remove expensive (time-wise) solutions early.
